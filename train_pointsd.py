...
from attention_processor import IPAttnProcessor2_0 as IPAttnProcessor, AttnProcessor2_0 as AttnProcessor, PCProjModel
...
class IPAdapter(torch.nn.Module):
    """IP-Adapter"""
    def __init__(self, unet, pc_model, global_proj_model, local_proj_model, adapter_modules, pc_model_frz=None, global_proj_model_frz=None):
        super().__init__()
        self.unet = unet
...
def parse_args():
    ...
    parser.add_argument(
        "--view_idx", type=int, default=0, help="select view idx."
    )
    parser.add_argument(
        "--depth", type=int, default=4, help="resampler depth."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--run_stage", type=str, default='stage1', choices=['stage1','stage2'], help="decide to run which stage."
    )
    parser.add_argument(
...
def main():
    ...
    if accelerator.distributed_type == DistributedType.DEEPSPEED:
        ...
        print("DEEPSPEED", "train_micro_batch_size_per_gpu", accelerator.state.deepspeed_plugin.deepspeed_config[
            "train_micro_batch_size_per_gpu"
        ])
    ...
    for i,name in enumerate(unet.attn_processors.keys()):
        ...
        if cross_attention_dim is None:
            ...
            attn_procs[name] = IPAttnProcessor(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim,scale=args.scale,num_tokens = args.num_tokens)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(ip_adapter):
                ...
                if to_mix > 0.5:
                    ...
                    pixel_values_cat_2 = torch.cat([batch["pixel_values"][:,1],batch["pixel_values"][:,0]],dim=-1)
                    pixel_values_pad = torch.ones_like(pixel_values_cat_1).to(pixel_values_cat_1.device)
                    pixel_values_cat_1 = torch.cat([pixel_values_pad[:,:,:h//2,:],pixel_values_cat_1, pixel_values_pad[:,:,h//2:,:]],dim=2)
                    pixel_values_cat_2 = torch.cat([pixel_values_pad[:,:,:h//2,:],pixel_values_cat_2, pixel_values_pad[:,:,h//2:,:]],dim=2)
                    batch["pixel_values"] = torch.cat([pixel_values_cat_1.unsqueeze(1),pixel_values_cat_2.unsqueeze(1)],dim=1).reshape(b,c,2*h,2*w)
                    batch["pixel_values"] = F.interpolate(batch["pixel_values"],size=(h,w))
                
                latents = vae.encode(batch["pixel_values"].to(dtype=weight_dtype)).latent_dist.sample().detach()
                latents = latents * vae.config.scaling_factor

                global_tokens,recon_feat,global_tokens_frz=ip_adapter.get_pc_embed(batch["pc"], to_mix>0.5)

                # Sample noise that we'll add to the latents
                noise = torch.randn_like(latents)
                bsz = latents.shape[0]
                # Sample a random timestep for each image
                if args.train_timesteps_min is not None and args.train_timesteps_max is not None:
                    timesteps = torch.randint(args.train_timesteps_min, args.train_timesteps_max, (bsz,), device=latents.device)
                else:
                ...
                if args.run_stage=='stage2':
                    timesteps=torch.zeros_like(timesteps).to(device=latents.device).long()
                ...
                encoder_hidden_states = torch.zeros([batch['pc'].shape[0],77,768]).to(device=latents.device, dtype=weight_dtype)
                
                if args.run_stage=='stage1':
                    model_pred, feat = ip_adapter(noisy_latents, timesteps, encoder_hidden_states, global_tokens)
                    loss_denoising = F.mse_loss(model_pred.float(), noise.float(), reduction="mean")
                    loss = loss_denoising
                elif args.run_stage=='stage2':
                    model_pred, feat = ip_adapter(noisy_latents, timesteps, encoder_hidden_states, global_tokens_frz)
                    target = feat[0].squeeze()
                    loss_alignment = F.mse_loss(recon_feat.squeeze(), target, reduction="mean")
                    loss = loss_alignment
